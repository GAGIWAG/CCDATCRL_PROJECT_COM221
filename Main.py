...
def display_lit_hit_data():
    ...
    for index,book_sorted_info in sorted_df.iterrows():
        ...
        button = customtkinter.CTkButton(lithit_firstShelf_frame,width=875,height=125,hover_color="#241811",fg_color="#2D1E16", text=book_text,font=("Quando",20), command=lambda info=book_sorted_info: display_book_details(info),corner_radius=0)
        button.pack(padx=15, pady=5)
        
    # Create buttons for each row and display them in the GUI
    for index,book_sorted_info in sorted_df.iterrows():
        book_text = f"{book_sorted_info['title']} | Rating :{book_sorted_info['rating']}"
        button = customtkinter.CTkButton(home_lithit_frame_inner,hover_color="#4d3d33",fg_color="#614D40", text=book_text,font=("Quando",35), command=lambda info=book_sorted_info: display_book_details(info),corner_radius=0)
...
def last_read_append(book_info):
    new_button = customtkinter.CTkButton(lRead_firstShelf_frame,width=985,hover_color="#B88B68",fg_color="#614D40",height=135, text=f"{book_info['title']} \nby {book_info['author']}",font=("Quando",35), command=lambda info=book_info: display_book_details(info),corner_radius=15)
    new_button.pack()
...
home_lastread_frame_inner = customtkinter.CTkFrame(home_lastread_frame_outer,fg_color="#B88B68",corner_radius=0)
